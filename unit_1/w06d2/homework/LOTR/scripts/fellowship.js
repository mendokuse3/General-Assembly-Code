...
const makeMiddleEarth = () => {
  ...
  for (let i = 0; i < lands.length; i++){
    ...
    $landArticle.append($('<h1>').text(lands[i]));
...
console.log('making baddies');
  ...
  for (let i = 0; i < baddies.length; i++){
    const $baddie = $('<li>').text(baddies[i]);
...
const beautifulStranger = () => {
  ...
  const $stranger = $('aside').children().children();
  $stranger.eq(3).text('Aragorn');
...
const forgeTheFellowShip = () => {
  ...
  $('.hobbit').parent().appendTo($fellowship);
  $('.buddy').parent().appendTo($fellowship);
};
...
const weWantsIt = () => {
  ...
  $('#Mordor > .hobbit > div').appendTo($gollum);
